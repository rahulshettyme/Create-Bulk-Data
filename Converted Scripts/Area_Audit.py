...
def run(rows, token, env_config):
    ...
    def process_row(row):
        ...
        try:
            ...
            if not ca_id:
                new_row['Status'] = 'Fail'
                new_row['API_Response'] = 'Missing CA_ID'
                return new_row
            ...
            if not coords:
                if min_lat is not None:
                    ...
                    new_row['Status'] = 'Fail'
                    new_row['API_Response'] = 'No Coordinates provided and no Boundary set'
                    return new_row
            ...
            if audit_resp.status_code == 200:
                new_row['Status'] = 'Pass'
                new_row['API_Response'] = audit_resp.json().get('message', 'Success')
